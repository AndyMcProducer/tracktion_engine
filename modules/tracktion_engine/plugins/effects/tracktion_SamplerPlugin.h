                      ...
                      private juce::AsyncUpdater
{
   public:
    ...
    bool isSoundReversed(int index) const;
    void setSoundReversed(int index, bool reversed);

    int getSoundMinVelocity(int index) const;
    int getSoundMaxVelocity(int index) const;
    void setSoundVelocityRange(int index, int minVel, int maxVel);

    int getSoundOutputIndex(int index) const;
    void setSoundOutputIndex(int index, int outputIndex);

    float getSoundAttack(int index) const;
    float getSoundDecay(int index) const;
    float getSoundSustain(int index) const;
    float getSoundRelease(int index) const;
    void setSoundAdsr(int index, float a, float d, float s, float r);

    // returns an error
    juce::String addSound(const juce::String& sourcePathOrProjectID, const juce::String& name, double startTime, double length, float gainDb);
    ...
    struct SamplerSound
    {
        ...
        bool reversed = false;
        int minVelocity = 0, maxVelocity = 127;
        int outputIndex = 0;
