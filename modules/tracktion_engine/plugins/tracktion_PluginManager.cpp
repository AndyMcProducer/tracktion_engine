...
inline namespace engine
{
        ...
        : engine (e)
    {
        ...
        {
            return std::unique_ptr<juce::AudioPluginInstance> (pluginFormatManager.createPluginInstance (description, rate, blockSize, errorMessage));
        };
    }
    ...
    void PluginManager::initialise()
    {
        ...
        auto scannerPtr = std::make_unique<PluginScanHelpers::CustomScanner> (engine);
        customScanner = scannerPtr.get();

        abortCurrentPluginScan = [c = scannerPtr.get()]
        {
            c->cancelScan();
        };

        knownPluginList.setCustomScanner (std::move (scannerPtr));
    ...
    juce::Array<juce::PluginDescription> PluginManager::getARACompatiblePlugDescriptions()
    {
        ...
        for (const auto& p : knownPluginList.getTypes())
        {
            ...
            // Check for common ARA plugin names or "ARA" tag in metadata
            if (p.name.containsIgnoreCase ("Melodyne")
                || p.name.containsIgnoreCase ("VocAlign")
                || p.name.containsIgnoreCase ("Revoice")
                || p.name.containsIgnoreCase ("SpectraLayers")
                || p.name.containsIgnoreCase ("Auto-Tune Pro") // Some versions are ARA
                || p.name.containsIgnoreCase ("Vovious") // User requested
                || p.name.containsIgnoreCase ("BPM") // User requested
                || p.name.containsIgnoreCase ("ARA")
                || p.category.containsIgnoreCase ("ARA")
                || p.descriptiveName.containsIgnoreCase ("ARA"))
            {
                isARA = true;
            }
            ...
            if (isARA)
            {
                // For Melodyne, we still want to ensure it's version 4 or higher (first ARA versions)
                if (p.name.containsIgnoreCase ("Melodyne"))
                {
    ...
    juce::KnownPluginList::CustomScanner* PluginManager::getCustomScanner() const
    {
    ...
    {
        return juce::jlimit (1, juce::SystemStats::getNumCpus(), static_cast<int> (engine.getPropertyStorage().getProperty (SettingID::numThreadsForPluginScanning, 1)));
    }
