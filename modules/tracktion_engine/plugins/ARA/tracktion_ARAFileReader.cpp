...
juce::String ARAFileReader::getPreferredPluginName (Engine& engine)
{
    return engine.getPropertyStorage().getProperty (SettingID::araPreferredPlugin, juce::String());
}

void ARAFileReader::setPreferredPluginName (Engine& engine, const juce::String& name)
{
...
struct ARADocumentHolder::Pimpl
{
    ...
    void initialise()
    {
        ...
        if (neededPluginKeys.empty())
        {
            ...
            {
                auto preferredName = ARAFileReader::getPreferredPluginName (edit.engine);
                juce::PluginDescription preferred;

                if (preferredName.isNotEmpty())
                    for (auto& d : defaultDescs)
                        if (d.name == preferredName || d.descriptiveName == preferredName)
                        {
                            preferred = d;
                            break;
                        }

                if (preferred.name.isEmpty())
                    preferred = ARAClipPlayer::ARAPluginFactory::findPreferredDefault (defaultDescs);
...
void ARAFileReader::cleanUpOnShutdown()                        {}
juce::String ARAFileReader::getPreferredPluginName (Engine&)    { return {}; }
void ARAFileReader::setPreferredPluginName (Engine&, const juce::String&) {}
