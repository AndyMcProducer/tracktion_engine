...
inline namespace engine
{
    ...
    void LevelMeasurer::processBuffer (juce::AudioBuffer<float>& buffer, int start, int numSamples)
    {
        ...
        if (processCallback)
        {
            const float* readPtrs[8];
            const int n = std::min (numChans, 8);
            for (int i = 0; i < n; ++i)
                readPtrs[i] = buffer.getReadPointer (i, start);
            processCallback (readPtrs, n, numSamples);
        }
        ...
        else
        {
            ...
            for (auto c : clients)
            {
                ...
                if (sum > 0.999f)
                    c->setOverload (0, true);
                if (diff > 0.999f)
                    c->setOverload (1, true);
